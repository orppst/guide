...
\begin{abstract}
    ...
    using the React framework, specifically leveraging the Mantine library \url{https://mantine.dev/}.
    We are also actively developing a CLI in Java to provide companion access to the API\@.

    The API, GUI and CLI are maintained in open-source repositories on GitHub \url{https://github.com/orppst}.
...
North Star, the current observation proposal tool used both by the radio and optical astronomy communities is at
end-of-life.
...
As part of work-package 2.1 of this pilot we are developing a new open source proposal tool that provides a single
...
Polaris.\footnote{North Star++ just didn't have the right "ring" to it.}





\subsection{The Author Checklist}\label{subsec:the-author-checklist}
The following checklist should be followed when writing a submission to a conference proceedings to be published by
the ASP for ADASS. The references are to sections in the ADASS manuscript instructions.\footnote{Most URLs should be
in a footnote like this one.  In this case, you can download the online material from \url{http://www.adass.org}.}
